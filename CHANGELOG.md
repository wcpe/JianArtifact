...
- 控制台制品搜索重构为页眉全局搜索 + 树形结果（FR-94，增强 FR-22/67，纯前端、不改后端）：把跨仓库制品搜索入口前移到页眉——FR-92 预留的禁用占位框换为**可用的全局搜索框**（输入关键字回车立即跳、停止输入防抖 300ms 后自动跳，跳转 `/search?q=`）。`SearchPage` 经 URL `q` 自动调既有 `GET /search` 端点（结果仍由后端按读权限过滤），命中由新增纯函数 `lib/searchTree.ts` 客户端折叠为**按仓库分组的可展开树**（分组按仓库名升序、组内按路径升序），每个仓库分组 / 命中按格式复用 `lib/formatIcon.tsx` 显示**专属 icon**，点击命中进 `/artifact` 详情；保留格式过滤与分页。搜索语义 / 权限过滤不改后端；未新增前端依赖
...
- 控制台设置页重构为单页堆叠 + 在线更新高级项折叠（FR-103，增强 FR-88/96/100，纯前端、不改后端）：设置页由「左侧页内 tab + 右侧表单」改为**单页纵向堆叠**——网络代理 → 在线更新 → 检查与应用更新 → 关于·版本 四块从上到下顺序排列、去掉 tab 切换，彻底消除切 tab 卡片忽大忽小、保存按钮跳位；底部保存条保持 sticky 固定（`data-testid="settings-save-bar"` 不变）。在线更新区**默认仅显「启用在线更新」开关 + 更新通道**，低频高级项（仓库源 owner/repo / API 基址 / 重启模式 / 访问令牌）收进默认收起的「高级设置」`Collapse` 折叠区、点开才显示编辑。检查更新结果除版本对比外，**展示拉取到的 release 发布说明**（`UpdateCheck.notes` 即 release body，无说明时优雅留空）。仅重排呈现：沿用 FR-88/89/100 既有数据加载 / 保存 / 检查 / 应用逻辑与代理三字段、token 三态、脱敏口径，不改 `GET` / `PATCH /api/v1/settings` 契约；未新增前端依赖
