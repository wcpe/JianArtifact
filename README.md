...
## 前端 Mock 模式（无需后端，开发 / 演示用）

前端内置一套**有状态内存 mock 后端**（基于 MSW），开启后由浏览器在前端内拦截全部 `/api/v1/*`、全操作走内存 CRUD，**无需启动真实后端**即可登录、创建仓库、浏览制品、造数据探索。预置示例数据，一开即用。**默认关闭、不影响生产**。

在 `frontend/` 下启动开发服务器，按需开启 Mock 模式（任一方式即可）：

- 构建 / 启动期 env：`VITE_MOCK=true pnpm dev`；
- 运行时 URL 参数：访问 `http://localhost:5173/?mock=1` 开启（落盘、刷新保留）、`?mock=0` 关闭；
- 页内开关：Mock 模式开启时右下角显示「Mock 模式」徽标，点其旁的关闭按钮即关闭并刷新回真实后端。

预置内置管理员账号 `admin / admin123` 供 Mock 模式登录。Mock 模式仅供本地开发 / 演示，生产构建默认不加载 worker 代码（经动态 import 隔离、按需懒加载）。
