// 多格式依赖坐标生成（FR-25b，纯前端纯函数）：
//
// 把 Maven 制品路径反解为 GAV，再用模板产出多种构建工具的依赖坐标片段
...
//
// 另含 HTML View 外链（指向目录索引视图）与制品下载 URL 的纯函数，适配本项目
// 协议层路径（/repository/{repo}/…）。
...
export function parseMavenGav(path: string): MavenGav | null {
  ...
  if (!version || !artifactId || groupSegments.length === 0) return null;
  return {
 ...
 * （npm / raw 等无统一 GAV，保留后端原生 usage 片段即可）。
 */
...
/**
 * HTML View 外链 URL（目录索引视图）。
 *
 * 指向制品所在目录的索引：`/repository/{repo}/{父目录}/`（尾斜杠 -> 后端按 Accept: text/html
 * 渲染目录索引页）。根目录文件回退到仓库根索引 `/repository/{repo}/`。
 */
export function htmlViewUrl(repoName: string, path: string): string {
  ...
  return `/repository/${repo}/${dir}/`;
}

/** 制品原始下载 URL：`/repository/{repo}/{path}`（逐段编码保留分隔斜杠）。 */
export function downloadUrl(repoName: string, path: string): string {
  return `/repository/${encodeURIComponent(repoName)}/${encodeSegments(path)}`;
}
