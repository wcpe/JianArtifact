...
- **格式 API**：错误遵循各自原生协议的约定（如 Docker registry v2 返回其规范的错误对象，Maven / npm / Raw / PyPI 主要以 HTTP 状态码表达），不套用上述统一 JSON 错误结构。限流 429 与黑名单 / 封禁 403 在格式 API 上同样可能发生（中间件在协议处理之前判定）。
