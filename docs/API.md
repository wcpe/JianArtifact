...
- **异步化（FR-125）**：上万 blob 的同步搬运会在前置反代后超时（504），故搬运改为**后台异步任务**——同步阶段只做在线枚举源仓库配置（失败即 400/502、不开任务），随后**立即返回 `202 + job_id`**，实际建仓 + 离线 blob 流式 ingest 在后台执行，进度经 `GET /api/v1/migrate/jobs/{id}` 轮询，**支持取消 / 暂停**（FR-91，每个 blob 与每个仓库边界响应）。
- **响应**：`202 Accepted`，体 `{ "job_id": string }`——供轮询 `GET /api/v1/migrate/jobs/{id}`。任务进度含 `total_assets` / `done_assets` / `migrated` / `skipped` / `current_repo` / `current_path` / `repos`（各仓库 `source_repo` / `target_repo` / `created` / `migrated_artifacts` / `skipped_artifacts`，proxy `target` 同源名）/ `skipped_repos`（因格式未实现或缺上游地址整体跳过）；`phase` 终态 `done` / `cancelled`（取消，已搬运保留）/ `failed`（`error` 载因）。
- **错误**（同步阶段）：`400` `offline_path` 为空；`401` 未认证；`403` 非管理员；`502` 连接 / 鉴权 / 解析源 Nexus 失败（在线枚举阶段）。离线目录不存在 / 缺 `content/` 等搬运期错误反映为任务 `failed` 阶段。
...
- **异步化（FR-125）**：同 proxy/migrate——同步只做在线枚举（失败即 400/502），随后**立即返回 `202 + job_id`**，搬运在后台任务执行、进度经 `GET /api/v1/migrate/jobs/{id}` 轮询、**支持取消 / 暂停**（FR-91）。
- **响应**：`202 Accepted`，体 `{ "job_id": string }`。任务进度结构同上（含各仓库 `migrated_artifacts` / `skipped_artifacts`（含路径非法 / 不可覆盖 / 超限）/ `skipped_repos`）；`phase` 终态 `done` / `cancelled` / `failed`。
- **错误**（同步阶段）：`400` `offline_path` 为空；`401` 未认证；`403` 非管理员；`502` 连接 / 鉴权 / 解析源 Nexus 失败（在线枚举阶段）。搬运期错误（缺 `content/` 等）反映为任务 `failed`。
...
- **行为**：**只读留存的上次检查结果，不联网**（页面打开即见，不触发 GitHub 限流、不必每次重新检查）。无留存时返回空结果（`result` 为 null）。
- **响应**：`{ "result": { "current_version", "latest_version", "update_available", "asset_name", "notes" } | null, "checked_at": <时间戳|null> }`。

### 触发在线更新检查（仅 Admin，FR-126）

- **方法 / 路径**：`POST /api/v1/update/check`
- **行为**：起后台联网检查 job（查 `[update] repo` 的 Release、按 `channel`（FR-89）取 stable/prerelease、比对当前版本），结果留存供 `GET /update/check` 读取。出站经 `[network.proxy]`（FR-84）；**`enabled=false` 时拒绝、不联网**；token 不进日志。
- **响应**：`202`，体 `{ "job_id" }`。进度 / 结果经 `GET /update/jobs/{id}` 轮询。
- **错误**：`401` 未认证；`403` 非管理员；`409` 在线更新未启用（`enabled=false`）。

### 应用在线更新（仅 Admin，FR-85）

- **方法 / 路径**：`POST /api/v1/update/apply`
- **请求**：无请求体。仅管理员可调用。
- **行为**：按本机平台取对应资产 → 流式下载并边算 sha256（不整体载入内存）→ 取同名 `.sha256` 资产比对 → 一致则**原子替换**当前二进制 → 置位重启请求触发优雅停机，排空在途请求后由 `main` 据 `[update] restart_mode`（`self` 自拉起 / `exit` 仅退出交外部进程管理器）拉起新进程或退出。**仅 sha256 校验通过才替换、替换是最后一步**：校验失败即删临时文件、保留旧二进制、进程续以旧版运行。`enabled=false` 时一律拒绝、不联网；token / 凭据不进日志、不回显。**资产选择顺序（FR-138）**：优先选压缩包（`jianartifact-{version}-{target}.zip` + `.zip.sha256` 均存在时）——下载 zip → 校验 sha256（针对 zip 整体）→ 解压取出二进制 → 原子替换；资产列表中无压缩包时回落裸可执行路径（向后兼容旧版发布）。
- **并发单飞**：apply 为进程级互斥，已有一次自更新在途时再次触发立即返回 `409`「更新进行中」，不竞争下载临时文件与替换中间态（`.bak`/`.old`/`.new`）；占用标志在更新结束（含出错 / 早返回）时可靠复位。check 端点不受影响。
- **响应（FR-126 异步）**：`202`，体 `{ "job_id" }`。下载 / 校验 / 替换在后台执行，进度经 `GET /update/jobs/{id}` 轮询；替换成功先留存终态再触发重启，重启后经 `GET /update/jobs` 回填上次 apply 终态续看。apply **不支持取消**（半截替换会坏二进制）。
...
- **响应（FR-126 异步）**：`202`，体 `{ "job_id" }`。还原 / 替换在后台执行，进度经 `GET /update/jobs/{id}` 轮询；返回后服务排空在途请求并按 `restart_mode` 重启。
- **错误**：`401` 未认证；`403` 非管理员；`409` 无可回滚的备份版本（从未成功升级过或备份缺失）/ 已有自更新在途（「更新进行中」）；`500` 本地替换失败。

### 更新任务进度（仅 Admin，FR-126）

- **方法 / 路径**：`GET /api/v1/update/jobs/{id}`（单任务）/ `GET /api/v1/update/jobs`（最近任务，含重启后从状态文件回填的上次 apply 终态）。
- **行为**：返回更新任务的进度快照——阶段（检查 / 下载 / 校验 / 替换 / 重启）、是否完成、结果 / 错误。**按阶段反馈、不返回假百分比**（根治旧版客户端模拟进度卡 95%）。`GET /update/jobs` 在二进制替换重启后仍能读到上次 apply 的终态（终态留存于 `{data_dir}/update-state.json`，token / 凭据不写入）。
- **响应**：`{ "job_id", "phase", "done", "result"?, "error"? }`。
- **错误**：`401` 未认证；`403` 非管理员；`404` 未知 `job_id`。
