...
- 统一出站网络代理（P2，FR-84 / ADR-0020）：全部出站 `reqwest::Client` 经 `config` 层共享 helper `build_outbound_client(timeout, &NetworkProxyConfig)` 构造，集中注入 `[network.proxy]`（`http` / `https` / `no_proxy`）正向出站代理与既有超时 / rustls / stream 特性。注入点：proxy 回源（`proxy::HttpUpstream::with_network`）、Nexus 在线迁移（`migrate::HttpNexusClient::with_network`）、漏洞库镜像（`vuln::HttpMirrorSource::with_network`）、OIDC（`main` 装配处经 helper 构造注入 `OidcProvider`）。helper 落在 `config`（所有出站模块共同最底层），不产生反向跨层依赖。env-vs-config：**配置给值即为真源**（注入即关闭 reqwest 的 auto-sys-proxy，压过系统环境）；**三键全空时不注入**，保持 reqwest 默认行为（含其 honor 系统 `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`），即「不配置等价现状」。代理 URL 可含凭据，凭据**不入库、不进日志 / 错误信息**（构造失败只记不含凭据的「代理配置无效」类消息）。
...
- ADR-0020 统一出站网络代理：新增 `[network.proxy]`（http/https/no_proxy + env 覆盖）为出站代理唯一真源，`config` 层抽共享出站客户端 helper 统一注入全部出站 reqwest 客户端（rustls 保持、代理凭据脱敏）；配置给值即为真源、不配置保留 reqwest 默认（含系统 env）。P2。
