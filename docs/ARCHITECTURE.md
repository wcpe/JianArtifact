...
- 管理 API：挂载于 `/api/v1/*`（涵盖 auth 登录/登出/刷新、当前用户 `me`、users、repositories（含制品浏览与删除）、acl、tokens、search、audit（仅 Admin，P2）、analytics/usage（仅 Admin，P2）、protection/status 与 protection/alerts（仅 Admin，P2）、health），采用 REST + JSON 风格，统一分页与错误约定（详见 API.md），供 Web 控制台与脚本使用。
