...
### [protection.rate_limit]（基础速率限制，P2 / FR-33 / ADR-0008）
...
| identity_max_requests | 单身份（用户及其所有 Token / 会话）每窗请求数上限；超过即对该主体返回 429 | 2400 | （经 TOML 配置） |

> 默认关闭，启用与调阈值由运维显式承担。默认阈值**保守宽放**，不误杀正常包管理器批量拉取（如 CI 高频拉取）；调小阈值前请评估正常峰值。来源 IP 取**连接级地址**、**不采信 `X-Forwarded-For`**（伪造来源不绕过）；若部署在反向代理之后，限流按代理与本服务之间的连接 IP 计数（见 OPERATIONS）。仅应用层（L7）基础限流；多维（用户 / 仓库）限流与并发/连接上限属 FR-51、慢速 / 封禁 / CC / WAF 属 FR-52~56，均未实现。按 TOML 嵌套节 `[protection.rate_limit]` 配置即可（本两层键以 TOML 为准，环境变量前缀仅对单层节名做嵌套映射）。
