...
| 文档                      | 管什么                                                                | 何时更新                     | 入库       |
| ------------------------- | --------------------------------------------------------------------- | ---------------------------- | ---------- |
| `docs/README.md`          | 文档入口、真源分层、当前版本口径和阅读路径                            | 文档结构或真源分层变化时     | ✓          |
...
4. 改 API.md                    接口契约变更
5. 实现 + 测试                  过验证门（后端 `task lint/test/vet/vuln`、前端 `turbo lint/typecheck/test/build` 均全绿）
...
版本号唯一来源是根 `VERSION` 文件，构建把它注入后端二进制与前端制品，恒一致。
