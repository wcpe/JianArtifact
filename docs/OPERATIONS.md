...
2026-09-13 的脱敏提交 `b640c33` 已把工作区与后续提交中的真实基础设施地址替换为 RFC 2606 文档域（`example.com` / `example.net` / `example.org`）与 RFC 1918 内网段；2026-09-20 补上了该提交漏掉的一处大小写变体（`REPO.Upper.Top` 输入端，见 `settings_test.go`）。

但 **git 历史是不可变的**：在 `b640c33` **之前**的旧提交里，以下 9 个真实值仍可从分支历史取回（含 2 个公网 IP 与 7 个 `*.wcpe.top` 系域名，涉及约 55 处提交）：

- 公网 IP ×2、`*.wcpe.top` 域名 ×7（`repo` / `maven` / `bak.maven` / `tmp` / `repo1` / `t2` 等子域）
