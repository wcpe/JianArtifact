...
| XXXX | group / 虚拟聚合仓库（FR-136，编号落地时分配）：新增第三种仓库类型 group——自身不存 blob，聚合有序成员仓库，GET 按 position 逐成员施加读判定、首个「有权且存在」命中即返回、只读（写删 405）；推翻 scope-discipline 对 group 的排除（用户已授权范围扩张），取代 ADR-0005「group 顺延第三期」。限制：不做跨成员 metadata 合并、Docker group 不在范围（属后续 FR） | 已接受 |
