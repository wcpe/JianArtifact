...
| 0037 | Maven 服务端权威 maven-metadata.xml + pom 三级兜底（client-priority，FR-121，取代 p1-maven「服务端不重写 metadata、由客户端自管」）：任一 Maven 主版本文件写入 hosted 后，服务端按 SQLite 索引（唯一真源）聚合该 `{groupId}/{artifactId}` 全部版本重生成 artifact 级 `maven-metadata.xml`（versions/latest/release/lastUpdated）+ 四校验和 sidecar（可幂等重写、无 409）；pom 三级兜底保证 `{a}-{v}.pom` 存在——客户端已传（不覆盖）→ jar 内嵌 pom 提取（FR-120）→ 按 GAV 生成最小 pom；client-priority 只补缺、绝不改写客户端已传内容；完整 SNAPSHOT 快照规则见 FR-122（P2） | 已接受 |
| 0038 | 统一进程内异步任务注册表（FR-131，修订 ADR-0019）：把 ADR-0019「单一迁移 job 注册表」扩为 kind-agnostic 统一任务模型（`api::TaskRegistry`），迁移 / 在线更新 / 漏洞库刷新三类长耗时任务统一登记轻量 `TaskRecord`（kind + 统一状态 running/paused/succeeded/failed/cancelled + 起止时间 + 可选 error），进度明细仍归各自专表（同 job_id 关联、不复制第二份真源）；**迁移单飞**（搬运同时只一个在途、第二个 409，离线预览不参与）、**有界历史保留**供找回；新增仅 Admin `GET /api/v1/tasks`(`/{id}`)，既有各 kind 端点契约不变；仍进程内不落库、重启即清（同 ADR-0019）；vuln 经 `RefreshObserver` 回调接入守分层不反向依赖（P2） | 已接受 |
| 0039 | group / 虚拟聚合仓库（FR-136）：新增第三种仓库类型 group——自身不存 blob，聚合有序成员仓库，GET 按 position 逐成员施加读判定、首个「有权且存在」命中即返回、只读（写删 405）；推翻 scope-discipline 对 group 的排除（用户已授权范围扩张），取代 ADR-0005「group 顺延第三期」。限制：不做跨成员 metadata 合并、Docker group 不在范围（属后续 FR） | 已接受 |
