...
- 不改版本比较语义：仍以 `major.minor.patch` 三段整数比较；预发布串的后缀（`-rc.1` 等）按既有规则忽略后缀比较（已有 `parse_version` 行为，不新增预发布优先级排序）。
- 不改下载 / 校验 / 替换 / 重启链路：通道只影响「取哪一条 release」，其后流程与 FR-85 完全一致。
...
- `build_check` / `apply_update` 编排不变，仅把 `channel` 透传给 `fetch_latest_release`。
...
- 版本比较对预发布串（`0.4.0-rc.1`）的处理与现状一致（忽略后缀比较）。
- 单元 / 集成测试绿；clippy `-D warnings`、fmt 过；前端 test + build 过。
- token / 凭据仍不进日志、不入库、不回显（沿用 FR-85 / FR-88 红线，本功能不触碰凭据）。

## 6. 风险 / 待定
- **「最新预发布」语义**：GitHub `/releases` 按发布时间倒序，取首个非 draft 即「最新」。若 prerelease 版本号低于已发布的 stable（回退预发布），仍按版本号比较决定 `update_available`（可能 `false`），符合「仅当更高才升级」的既有防御。
- **真机维度**：FR-85 的下载→替换→重启端到端真机验收不受本功能影响；本功能只改「取哪条 release」，新增逻辑均以 fake 源单测覆盖，无新增真机项。
