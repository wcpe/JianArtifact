...
use super::{MigrateError, NexusRepoSummary};
...
fn map_nexus_format(nexus_format: &str) -> Option<&'static str> {
...
fn normalize_blob_path(blob_name: &str) -> &str {
...
pub async fn migrate_proxy_repositories<S: BlobStore, U: Upstream>(
    ...
    for src in source_repos {
        ...
        let (migrated, skipped) =
            migrate_repo_artifacts(artifacts, formats, &repo, &repo_entries).await;
